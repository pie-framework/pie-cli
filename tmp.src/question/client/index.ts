...
export class ClientBuildable {
  ...
  get externals() {
    throw new Error('todo'); //return this._supportConfig ? this._supportConfig.externals : { js: [], css: [] };
  }
  ...
  // writeEntryJs() {
  //   let js = this.client.entryJs(this.config.declarations);
  ...
  _buildFrameworkConfig() {
    ...
    let appDependencyKeys = _.keys(this.client.dependencies(this.opts.pieBranch));
  ...
  _install() {
    let dependencies = _.extend({},
      ...
      this.client.dependencies(this.opts.pieBranch));
