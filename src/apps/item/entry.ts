...
export default function js(buildInfo: PieBuildInfo[],
  sockPath: string) {

  const controllerPies = buildInfo.filter(bi => bi.controller);
  return `
/** Auto generated by ${__filename} */

//pie controllers
let controllers = {};
${controllerPies.map(bi => controllerDependency(bi.element, bi.controller.moduleId)).join('\n')}

//pie declarations
${buildInfo.map(bi => new ElementDeclaration(bi.main.tag, bi.main.moduleId).js).join('\n')}
