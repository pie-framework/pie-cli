...
export function packExample(testName, exampleQuestion, support) {
  ...
  return FrameworkSupport.bootstrap(questionPath, support.concat(example.frameworkSupport()))
    .then(fs => {
      logger.info('FrameworkSupport: ', fs);

      let question = new Question(questionPath, Question.buildOpts(), fs, example);
